...
\begin{tabular}[c]{@{}l@{}}Zurück Koperien der Temporären \\ Arrays in das dst-Array\end{tabular} & 2.000                       & 108 \mu s            \\ 
\end{tabular}
\end{table}

Theoretisch könnte direkt auf den übergebenen Daten operiert werden.
Dies würde zu einer Theoretischen Zeitersparniss von $709 \mu s$ führen.
Dazu müsste aber die Definition der \textit{Dual}-Zahlen innerhalb der \textit{ForwardDiff.jl} Bibliothek umgeschrieben werden.
Dies ist allerdings nicht besonders leicht und würde vorraussetzen große Abschnitte der bereits verhandenen implementierungen umzuschreiben.
